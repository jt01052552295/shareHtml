...
const templateData = {
  ...
  filePath: (basePath + '/assets/file').replace(/\/{2,}/g, '/')
};
...
// 파일 별도 복사
function file() {
  return src(paths.file.src).pipe(dest(paths.file.dest));
}
...
function serve() {
  ...
  watch(paths.file.src, series(file, reload));
}
